...
# Functions are methods in the class operating on the DNA string 
# and the exon regions stored in the class.
...
# Simple constructor expects the exon regions to be specified as a list 
# of (start, end) tuples indicating the start and end of each region:
...
# Exon regions may not be known, so we allow None as value (used as default value).
# Exon regions at the start and/or end of the DNA string will lead to empty intron 
# Region objects so a proper test on nonzero length of the introns must be inserted.
# Data for the DNA string and the exon regions can either be passed as arguments or 
# downloaded and read from file. Two different initializations as follows:
...
# Methods for creating a mutated gene:
...
# Remove leading underscore; signals these attributes are “protected” (not to be directly accessed by the user)
...
# Two fundamental types of genes: 1.) codes for proteins (via mRNA) 2.) only codes for RNA
...
# Example: Load the lactase gene and create the lactase protein
